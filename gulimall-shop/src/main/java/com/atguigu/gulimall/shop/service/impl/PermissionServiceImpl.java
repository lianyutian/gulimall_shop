...
public class PermissionServiceImpl implements PermissionService {
    ...
    private List<TreeModel> getSelectMenuTree(List<SysPermission> sysPermissionList) {
        if (CollectionUtils.isEmpty(sysPermissionList)) {
            return Collections.emptyList();
        }
        List<TreeModel> selectMenuTree = new ArrayList<>();
        sysPermissionList.forEach(permission -> {
            // 判断是否目录
            if (permission.getType().equals(Constant.Type.CATALOG.getValue())) {
                TreeModel selectMenu = new TreeModel();
                selectMenu.setId(permission.getId());
                selectMenu.setLabel(permission.getName());
                List<TreeModel> childMenu = getChildMenu(permission.getId(), sysPermissionList);
     ...
     * @return 子权限
     */
    private List<TreeModel> getChildMenu(String id, List<SysPermission> sysPermissionList) {
        List<TreeModel> childMenuTree = new ArrayList<>();
        sysPermissionList.stream().filter(permission -> permission.getPid().equals(id) && permission.getType() != Constant.Type.BUTTON.getValue()).forEach(permission -> {
            TreeModel childMenu = new TreeModel();
    ...
    @Override
    public List<TreeModel> getSelectMenuTree() {
    ...
    private List<SysPermission> getPermissions(List<SysPermission> permissions) {
        ...
        for (SysPermission permission : permissions) {
            ...
            // 目录
            if (permission.getType() == Constant.Type.CATALOG.getValue()) {
